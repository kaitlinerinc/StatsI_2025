...
\begin{document}
	...
	\begin{enumerate}
		...
		\noindent First I put the tabular data into a matrix in R and added the totals row and column. I labeled the matrix the same way the original table was labeled.
		
		\lstinputlisting[language=R, firstline=7, lastline=28]{PS02_answersKC.R}  
	
	\begin{center}
		\begin{verbatim}
            Not Stopped Bribe Requested Stoped/Given Warning
...
\end{itemize}
	...
	\item Null hypothesis: the reservation policy has no affect on the number of drinking water facilities
    \item Alternative hypothesis: the reservation policy has an affect on the number of drinking water facilities
		...
		\noindent Use the lm function in R to run the regression, I subset two columns from the original data set to input only the two variables that I am investigating the relationship between, reserved and water.
...
F-statistic: 5.493 on 1 and 320 DF,  p-value: 0.0197
   ...
   \begin{itemize}     
        ...
        \item A y-intercept of 14.738 suggests that there are an average of 14.738 new or repaired drinking-water facilities when there is no reservation policy in place.
    \end{itemize}
